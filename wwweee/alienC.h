...
/**
 * @brief Klasa reprezentuj¹ca trzeci typ obcego (Alien C).
...
public:
    ...
    /**
     * @brief Konstruktor klasy alienC.
     * @param edge KrawêdŸ, z której obcy siê pojawi (1: lewo, 2: prawo, 3: góra).
     * @param type Typ obcego (dla alienC zawsze 0, ale zachowane dla spójnoœci z klas¹ bazow¹).
    ...
    /**
     * @brief Statyczna metoda do ³adowania wszystkich tekstur dla Alien C.
     * * Tekstury s¹ ³adowane tylko raz i przechowywane w statycznej tablicy `alienCImages`.
     */
    static void LoadImages();

    /**
     * @brief Statyczna metoda do roz³adowywania wszystkich tekstur dla Alien C.
     * * Zwolnienie zasobów tekstur.
     */
    static void UnloadImages();

    /**
     * @brief Statyczna tablica przechowuj¹ca tekstury dla Alien C.
     * * Tablica zawiera trzy tekstury odpowiadaj¹ce ró¿nym krawêdziom (edge).
     */
    static Texture2D alienCImages[3];

    /**
     * @brief Statyczna metoda zwracaj¹ca referencjê do tekstury Alien C na podstawie krawêdzi.
